...
def yolov8_configuration():
    ...
    yolov8_config["iou_thres"] = iou_thres
    yolov8_config["device"] = device
...
def yolov8_detect():
    ...
    if request.form['model'] == 'yolov8':
        model = YOLOv8(yolov8_config['weights'], yolov8_config['classes'])
...
def upload():
    ...
    # # 注意：没有的文件夹一定要先创建，不然会提示没有该路径 #保存Original Image至./static/uploads
...
@app.route("/yolov8_detect", methods=['POST'])
def detect():
    ...
    for upload_file_path in upload_files_path:
        if IMAGE_FLAG:
            if y57_c:
                ...
                print("Configuring YOLOX parameters")
            ...
            # 调整Keyframe Extraction过程的Parameter
            if kf_c:
...
def get_repairAdvice():
        ...
        if excel_path:  # 正常通过提取公式 Deformation
            ...
            try:
                ...
                if VIDEO_FLAG:
                    ...
                    temp_dict["attribute"] = "Video frame order"
                ...
                for line in file_object:
                    ...
                    if v[0] == "E":
                        ...
                        temp_dict["attribute"] = v[0] + "(Pipeline importance parameters, default values:3)"
                    elif v[0] == "K":
                        # temp_dict["attribute"] = v[0] + "(Regional importance parameter,default:6)"
                        temp_dict["attribute"] = v[0] + "(Regional importance parameter, default value:6)"
                    elif v[0] == "T":
                        # temp_dict["attribute"] = v[0] + "(Soil quality influence parameter,default:8)"
                        temp_dict["attribute"] = v[0] + "(Soil quality influencing parameters, default values:8)"
                    elif v[0] == "smax":
                        # temp_dict["attribute"] = v[0] + "(Pipeline section damage condition parameter, the most severe punishment value)"
                        temp_dict["attribute"] = v[0] + "(Pipeline area damage situation parameters, maximum value)"
                    elif v[0] == "s":
                        # temp_dict["attribute"] = v[0] + "(Pipe section damage condition parameter, average score)"
                        temp_dict["attribute"] = v[0] + "(Pipeline area damage situation parameters, average value)"
                    elif v[0] == "f":
                        # temp_dict["attribute"] = v[0] + "(Structural defect parameters of pipe section)"
                        temp_dict["attribute"] = v[0] + "(Pipeline area structural defect parameters)"
                    elif v[0] == "ri":
                        # temp_dict["attribute"] = v[0] + "(Segment repair index)"
                        temp_dict["attribute"] = v[0] + "(Segmental Repair Index)"
                    elif v[0] == "diameter":
            ...
            if VIDEO_FLAG:
                ...
                advice_list.append(temp_dict)
            temp_dict = {}
            temp_dict["attribute"] = "Hint"
            temp_dict["value"] = "There is no defect in the image or the defect type is under development"
            advice_list.append(temp_dict)
                ...
                for line in file_object:
                    ...
                    if v[0] == "E":
                        ...
                        temp_dict["attribute"] = v[
                                                     0] + "(Pipeline section damage condition parameter, )"
                    elif v[0] == "s":
                        temp_dict["attribute"] = v[0] + "(Pipe section damage condition parameter, average score)"
                    elif v[0] == "f":
                        temp_dict["attribute"] = v[0] + "(Structural defect parameters of pipe section)"
    ...
    if result==False:
        temp_neo4j_node=Node("Pipe", deformArea=temp_dict_neo4j["deformArea"],diameter=temp_dict_neo4j['diameter'],grade=temp_dict_neo4j['grade'],
                             E=temp_dict_neo4j['E'],K=temp_dict_neo4j['K'],T=temp_dict_neo4j['T'],score=temp_dict_neo4j['score'],uri=temp_dict_neo4j['uri'],
    """_summary_
...
# MATCH (n:Pipe{uri: 'image1'}) DETACH DELETE (n) 直接在neo4j中delete
