...
def find_max_proportion_lists(all_content_dict):
    ...
    for k, v in all_content_dict.items():
        ...
        PL_list, Deformation_list, TJ_list, CK_list = make_class_list(v)
        # 找出其中各自占比最大得
    ...
    return files_max_PL_list, files_max_Deformation_list, files_max_TJ_list, files_max_CK_list
...
def make_class_list(mix_class_list):
    ...
    for i in mix_class_list:
        if i[0] == 0:
            ...
            CK_list.append(i)
    return PL_list, Deformation_list, TJ_list, CK_list
...
def delete_excel():
    ...
    if len(excel_files) != 0:
        for i in tqdm(excel_files, desc="delete旧文件ing"):
