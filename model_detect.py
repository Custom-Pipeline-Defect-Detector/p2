...
def run_yolov_5_7(
    ...
    # 额外加入，（1）记录test results的图像路径（2）对labels文本信息进行处理
    # 将test results图像的路径保存到txt文件中
    ...
    for i in tqdm(all_content_dict, desc="生成新excel文件ing"):
        if len(files_max_Deformation_list) != 0:
            write_excel(files_max_Deformation_list, save_dir)
        ...
        if len(files_max_Obstacle_list) != 0:
            write_excel(files_max_Obstacle_list, save_dir)
...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        bboxes /= ratio  # Original Image中缺陷bounding box的左上坐标和右下坐标
        cls = output[:, 6]  # 缺陷类别
