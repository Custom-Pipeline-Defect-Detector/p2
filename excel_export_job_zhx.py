...
PL_4_PROPORTION = 0.2
# 3及变形，15-25%的形变
Deformation_3_PROPORTION = 15
# 4及变形，25%以上的形变
Deformation_4_PROPORTION = 25
# pai
PI = 3.14
# 直径
DIAMETER = 1000
...
def find_max_proportion_lists(all_content_dict,defect_img=None):
    ...
    # PL0 ，Deformation1， TJ2 ， CK3 ， ZAW4 ， CJ5
    # 所有文件的pl Deformation tj ck各自集合的二维list， 其内容为max_PL_list 加上文件名,返回值
    files_max_PL_list = []
    files_max_Deformation_list = []
    files_max_TJ_list = []
    files_max_CK_list = []
    files_max_Obstacle_list = []
    files_max_CJ_list = []

    for k, v in all_content_dict.items():
        # 一张图片里面各个病害类型的各自的list
        PL_list, Deformation_list, TJ_list, CK_list, Obstacle_list, CJ_list = make_class_list(v)
        # 找出其中各自占比最大得
        max_PL_list = get_max_proportion(PL_list)
        max_Deformation_list = get_max_proportion(Deformation_list)
        max_TJ_list = get_max_proportion(TJ_list)
        max_CK_list = get_max_proportion(CK_list)
        max_Obstacle_list = get_max_proportion(Obstacle_list)
        ...
        if len(max_Deformation_list) != 0:
            attribute_list = export_attribute_formula(defect_type=1, defect_list=max_Deformation_list)
            max_Deformation_list.append(str(k) + ".img")
            max_Deformation_list.extend(attribute_list)
            # print(max_Deformation_list)
            files_max_Deformation_list.append(max_Deformation_list)
        ...
        if len(max_Obstacle_list) != 0:
            attribute_list = export_attribute_formula(defect_type=5, defect_list=max_Obstacle_list)
            max_Obstacle_list.append(str(k) + ".img")
            max_Obstacle_list.extend(attribute_list)
            files_max_Obstacle_list.append(max_Obstacle_list)



    return files_max_PL_list, files_max_Deformation_list, files_max_TJ_list, files_max_CK_list, files_max_Obstacle_list, files_max_CJ_list
...
def export_attribute_formula(defect_type, defect_list, defect_img=None):
    ...
    # PL0 ，Deformation1， TJ2 ， CK3 ， CJ4 ， ZAW5
    # PL 得提取公式
    if defect_type == 0:
        # 置信度 defect_list[9]
        # 画面宽度
...
def make_class_list(mix_class_list):
    ...
    for i in mix_class_list:
        if i[0] == 0:
            ...
            CJ_list.append(i)
    return PL_list, Deformation_list, TJ_list, CK_list, Obstacle_list, CJ_list
...
def delete_excel():
    ...
    if len(excel_files) != 0:
        for i in tqdm(excel_files, desc="deleteexcel文件ing"):
            excel_file_path = os.path.join(excel_dir, i)
            os.remove(excel_file_path)
            time.sleep(1)
    else:
        time.sleep(1)
    print("===============旧excel文件delete完毕！=====================")
    time.sleep(2)
...
def write_excel(list, excel_path):
    ...
    if list[0][0] == 0:  # labels.txt第一个字符表示管道缺陷类别
        ...
        workbook.save(os.path.join(excel_dir, "Rupture.xls"))
        return os.path.join(excel_dir, "Rupture.xls")
    # Deformation
    elif list[0][0] == 1:
        workbook = xlwt.Workbook()
        worksheet = workbook.add_sheet("Deformation worksheet")
        worksheet.write(0, 0, 'Class')
        worksheet.write(0, 1, 'Instance')
        ...
        workbook.save(os.path.join(excel_dir, "Deformation.xls"))
        return os.path.join(excel_dir, "Deformation.xls")
    # TJ
    elif list[0][0] == 2:
        workbook = xlwt.Workbook()
        ...
        workbook.save(os.path.join(excel_dir, "Disconnect.xls"))
        return os.path.join(excel_dir, "Disconnect.xls")
    # CK
    elif list[0][0] == 3:
        workbook = xlwt.Workbook()
        ...
        workbook.save(os.path.join(excel_dir, "MIsalignment.xls"))
        return os.path.join(excel_dir, "MIsalignment.xls")
    elif list[0][0] == 4:
        workbook = xlwt.Workbook()
        ...
        workbook.save(os.path.join(excel_dir, "Deposition.xls"))
        return os.path.join(excel_dir, "Deposition.xls")
    elif list[0][0] == 5:
        workbook = xlwt.Workbook()
        ...
        workbook.save(os.path.join(excel_dir, "Obstacle.xls"))
        return os.path.join(excel_dir, "Obstacle.xls")
...
if __name__ == '__main__':
    ...
    files_max_PL_list, files_max_Deformation_list, files_max_TJ_list, files_max_CK_list = find_max_proportion_lists(
        all_content_dict)
    for i in tqdm(all_content_dict, desc="生成新文件ing"):
        if len(files_max_PL_list) != 0:
            write_excel(files_max_PL_list)
        if len(files_max_Deformation_list) != 0:
            write_excel(files_max_Deformation_list)
        if len(files_max_TJ_list) != 0:
            write_excel(files_max_TJ_list)
        if len(files_max_CK_list) != 0:
            write_excel(files_max_CK_list)
        if len(files_max_Obstacle_list) != 0:
            write_excel(files_max_Obstacle_list)
